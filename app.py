...
def analyze_comment(comment):
    ...
    # This gives both sentiment label and confidence score
    sentiment = sentiment_result["label"]
    sentiment_score = sentiment_result["score"]  # 0 to 1

    # Zero-shot classification for category
    category_result = category_classifier(comment, candidate_labels=CATEGORIES)
    category = category_result["labels"][0]  # top predicted category

    # Priority can also be predicted with zero-shot classification
    priority_result = category_classifier(
        comment,
...
def admin():
    ...
    high_priority = CustomerComment.query.filter_by(priority="High").count()
    negative_comments = CustomerComment.query.filter_by(sentiment="Negative").count()
...
def comment():
    if request.method == "POST":
        ...
        flight_id = request.form.get("flight_id", "N/A")
    ...
    comments = CustomerComment.query.order_by(CustomerComment.created_at.desc()).all()
